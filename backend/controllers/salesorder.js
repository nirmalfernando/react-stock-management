...
export const addSalesOrder = (req, res) => {
  ...
  db.query(salesOrderQuery, salesOrderValues, (err, result) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Failed to insert into Sales Order table" });
    }
    ...
    db.query(
      ...
      (err, result) => {
        if (err) {
          ...
          return res.status(500).json({ error: "Failed to insert into Sales Order Details table" });
        }
